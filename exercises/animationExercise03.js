...
const cylinderGeometryTwins = new THREE.CylinderGeometry(0.07, 0.07, 1.0, 25);
const cylinderMaterialTwins = new THREE.MeshPhongMaterial( {color:'rgb(255,250,0)'} );
const cylinder3 = new THREE.Mesh( cylinderGeometryTwins, cylinderMaterialTwins );
const cylinder4 = new THREE.Mesh( cylinderGeometryTwins, cylinderMaterialTwins );
cylinder3.rotate
...
function rotateCylinder()
{
  ...
  if(animationOn)
  {
    ...
    // Will execute T1 and then R1 - Rotation on parent cylinder
    cylinder.matrix.multiply(mat4.makeRotationZ(angle)); // R1
    cylinder.matrix.multiply(mat4.makeTranslation(0.0, 1.0, 0.0)); // T1

    // Will execute R2, T1 and R1 in this order - positioning and rotating cylinder 2
    ...
    cylinder3.matrix.multiply(mat4.makeTranslation(.5, 0, 0.0)); // T1
    cylinder3.matrix.multiply(mat4.makeRotationX(angle2))


    cylinder4.matrix.multiply(mat4.makeRotationZ(THREE.MathUtils.degToRad(90)));
    cylinder4.matrix.multiply(mat4.makeTranslation(-.5, 0, 0.0)); // T1
